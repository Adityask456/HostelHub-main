...
export default function AuthProvider({ children }) {
  ...
  const normalizeUser = (u) => {
    ...
    return {
      ...
      roomNumber: u.roomNumber ?? u.roomnumber ?? null,
  ...
  const login = async (email, password) => {
    try {
      ...
      // Save normalized user data to state
  ...
  const loadUser = async (showLoading = true) => {
    try {
      ...
      await AsyncStorage.setItem('@HostelHub:user', JSON.stringify(normalized));
  ...
  return (
    <AuthContext.Provider value={{ user, loading, login, logout, setUser, refreshUser: () => loadUser(false) }}>
      {children}
