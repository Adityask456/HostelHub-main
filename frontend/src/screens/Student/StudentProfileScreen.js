...
export default function StudentProfileScreen() {
  ...
  const load = async () => {
    try {
      ...
      // Normalize room number field (server may return `roomnumber`)
      const server = res?.data || null;
      if (server) {
        const normalized = { ...server, roomNumber: server.roomNumber ?? server.roomnumber ?? null };
        setMe(normalized);
      } else {
        setMe(ctxUser || null);
      }
  ...
  const openEdit = () => {
    setName(me?.name || "");
    setRoomNumber(me?.roomNumber ? String(me.roomNumber) : "");
    setEditOpen(true);
  };
    ...
    <SafeAreaView style={styles.root}>
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View style={styles.section}>
          ...
          <TouchableOpacity style={[styles.actionCard, { marginTop: 12 }]} onPress={handleLogout}>
      ...
      <Modal visible={editOpen} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.modalSub}>Update your details</Text>
