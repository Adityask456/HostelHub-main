      ...
      {expanded && (
        <View style={styles.pollOptions}>
          ...
          {poll.options.map((option, index) => {
            ...
            const isSelected = selectedOption === index || option.isUserChoice === true;
        ...
        const newPolls = prevPolls.map(poll => {
          if (poll.id === pollId) {
            const updatedOptions = (poll.options || []).map((opt, idx) => ({
              ...
              isUserChoice: idx === optionIndex
            }));
            return { ...poll, options: updatedOptions, hasVoted: true, userVote: updatedOptions[optionIndex]?.text };
          }
          return poll;
        });
